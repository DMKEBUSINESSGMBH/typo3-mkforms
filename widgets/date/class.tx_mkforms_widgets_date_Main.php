...
class tx_mkforms_widgets_date_Main extends formidable_mainrenderlet
{
    ...
    protected function _render()
    {
        ...
        $sTrigger = " <img src='".T3General::getIndpEnv('TYPO3_SITE_URL').$this->sExtRelPath."res/lib/js_calendar/img.gif' id='".$sTriggerId."' style='cursor: pointer;' alt='Pick date' /> ";
    ...
    protected function _includeLibraries()
    {
        ...
        $sLangFile = T3General::getIndpEnv('TYPO3_SITE_URL').$this->sExtRelPath.'res/lib/js_calendar/lang/calendar-'.$sLang.'.js';
        ...
        if ($cssFile = $this->getForm()->getConfTS('renderlets.date.css')) {
            ...
            $cssFile = $oJsLoader->getScriptPath(T3General::getIndpEnv('TYPO3_SITE_URL').$cssFile, 'css');
            $css = empty($cssFile) ? '' : '<link rel="stylesheet" type="text/css" media="all" href="'.$cssFile.'" />';
        }

        $oJsLoader->additionalHeaderData(
            (
                '<script src="'.$oJsLoader->getScriptPath(T3General::getIndpEnv('TYPO3_SITE_URL').$this->sExtRelPath.'res/lib/js_calendar/calendar.js').'"></script>'.
                '<script src="'.$oJsLoader->getScriptPath($sLangFile).'"></script>'.
                '<script src="'.$oJsLoader->getScriptPath(T3General::getIndpEnv('TYPO3_SITE_URL').$this->sExtRelPath.'res/lib/js_calendar/calendar-setup.js').'"></script>'.
                $css
            ),
